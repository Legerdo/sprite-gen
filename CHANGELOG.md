...
- `video-loop --anchor feet` no longer sways a walk. It pinned every cell on that frame's own foot line, which in an in-place gait jumps to the planted foot each step, so a body that stood still lurched back and forth by the stride. It now fits a straight line to the body's centre across the cycle and removes only that drift, keeping the step as performed; every cell stands on the mean foot line. `drift_px` now means the drift removed, and the strip meta adds `foot_sway_px` (the planted foot's in-gait movement, reported but not erased). Default `none` is unchanged.
...
- `video-loop --anchor feet` (also `video-set --anchor`): re-centre every strip cell on that frame's own foot line so a subject the model walked across an "in-place" canvas stops sliding once per cycle. Strip meta gains `foot_anchor`, `drift_px` and `foot_x`, and declares the spec loader's `anchor` as `[foot_x, h]` so scenes stand the sprite on its feet; the video-set table row carries `drift_px`. Default `none` keeps existing output byte-identical.
